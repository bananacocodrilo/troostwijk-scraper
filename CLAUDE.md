...
| `vat.py` | VAT-scheme detector — classifies each listing margin/incl/excl/vat_deductible/unknown and computes a private-buyer gross price (`price_gross_eur`) at a flat 21% |
...
- **Export JSON button** (`#exportBtn`, both dashboards): downloads the **currently shown** listings (after all filters AND the dismiss/bookmark override layer — it serialises the same `shownListings` array `render()` paints) to a structured evaluation schema (`basic`/`configuration`/`technical`/`condition`/`camper_relevance`/`financials`/`metadata` + a `fleetview_signals` block carrying our computed score/risk/market/cost intel). Fields we can't source are `null`/`"unknown"`. Derivations: make/model from `matched_token` (via `TOKEN_MAKE`), `standing_height`/`body_type` from `variant`, `layout`/`power_hp`/`transmission`/`engine` parsed from title+remarks. **`seats` is structured-only** (never text-inferred — see size/seat no-guess rules). It's a pure client-side feature; no backend involvement.
- **VAT detection** (`vat.py`, called from `asking_feed.build_feed`): every listing gets `vat_scheme` (margin/incl/excl/vat_deductible/unknown), `price_gross_eur` (private-buyer price), `vat_evidence`, `vat_confidence`. Detection precedence: margin (guarded against "geen/zonder/kein/no marge" negation) → explicit excl → explicit incl → reclaimable-VAT wording (`btw verrekenbaar`/`MwSt ausweisbar`/`TVA déductible` → treated **aggressively as excl**) → structured `vat_hint` from the scraper → lease-source default (`regeljelease`/`financiallease`/`rosfinance` → excl) → unknown. **Flat 21%** for all markets (DE is really 19% — per instruction use 21%). `price_gross_eur = round(price*1.21)` only for excl/vat_deductible/lease-default; otherwise = `price_eur`. **Label-only by default**: cohort medians/percentiles stay on raw `price_eur` — VAT is display + export metadata, not scoring input. Dashboards (`index.html`+`asking.html`) show a muted scheme badge per card and a `#vatInclFilter` ("💶 VAT-incl") toggle (off by default) that client-side flips shown price + recomputes % vs median on the gross basis. Scrapers capture a defensive `vat_hint` (marktplaats/2dehands scan btw/vat/mwst attrs; autotrack `totaalInclusiefBtw`→incl; mobile.de price.gross→incl; autoscout24 priceType probe) that improves accuracy as the cron re-populates `price_cache.json`. Export (`toExport`) carries `vat_scheme` + `price_gross_eur` alongside raw `price_eur`.
